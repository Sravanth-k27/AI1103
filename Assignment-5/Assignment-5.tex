...
Let the two students be A,B.
Let the events of solving the problem by A,B be X,Y respectively.
\\ The probability of solving the problem by A is $\pr{X}.$
 \\The probability of solving the problem by B is $\pr{Y}.$\\
Given that 
\begin{align}
...
As,
\begin{align}
    \pr{X+Y}=\pr{X}+\pr{Y}-\pr{XY}\label{eq:0.0.3}
\end{align}
Given that students solve the problem independently.So events X and Y are independent.\\
...
\begin{align}
    ...
    \end{align}
    \begin{align}
    \pr{XY}&=\frac{12}{25}\label{eq:0.0.6}
\end{align}
\\
now substituting \eqref{eq:0.0.1},\eqref{eq:0.0.2}, \eqref{eq:0.0.6} in \eqref{eq:0.0.3} gives 
\begin{align}
...
\pr{X+Y}=\frac{23}{25}
\end{align}
\\ Hence the probability that atleast one of them solves the problem is $\frac{23}{25}$\\
